...
## 🌟 ตัวอย่างจำลองสถานการณ์จริงทีละขั้นตอนสำหรับมือใหม่ (Beginner's Step-by-Step Walkthrough)

เพื่อให้เห็นภาพชัดเจนที่สุด ลองมาดูตัวอย่างการทำงานจริงทีละวินาที:

### สถานการณ์สมมุติ:
ผู้ใช้เปิด Terminal แล้วพิมพ์สั่ง **nanopi** ว่า:  
> **`"ช่วยแก้คำว่า bug เป็น fix ในไฟล์ index.html ให้หน่อย"`**

---

### เส้นทางการทำงานภายใน 5 ไฟล์ (ใช้เวลาจริงประมาณ 5 วินาที):

1. **วินาทีที่ 0 (รับคำสั่ง):**  
   - **`src/tui.ts` (โต๊ะประชาสัมพันธ์):** ใช้ `readline` รับข้อความที่คุณพิมพ์เข้ามา
   - ปรับสถานะเป็น **`busy = true`** ทันที เพื่อล็อกไม่ให้ผู้ใช้พิมพ์ข้อความใหม่แทรกขณะกำลังทำงาน

2. **วินาทีที่ 1 (จัดระเบียบส่งต่อ):**  
   - **`src/cli.ts` (ปลั๊กพ่วงรวมสาย):** นำข้อความของคุณจัดเป็น Message แบบ User แล้วเพิ่มเข้าไปใน **`Context` (สมุดโน้ต)**
   - สร้าง `AbortController` อันใหม่ เพื่อคอยดักฟังว่าคุณจะกด `Ctrl+C` กลางคันหรือไม่
   - สั่งเรียกฟังก์ชัน **`runAgent()`** ใน `src/agent.ts`

3. **วินาทีที่ 2 (ถามสมอง AI):**  
   - **`src/agent.ts` (โฟร์แมนคุมงาน):** เริ่ม `while (true)` ลูปแรก แล้วเรียก `stream()` ใน **`src/llm.ts`**
   - **`src/llm.ts` (ล่ามแปลภาษา):** ยิง HTTP POST ไปที่ API ของโมเดล พร้อมแนบประวัติในสมุดโน้ต
   - สมอง AI วิเคราะห์แล้วตอบกลับมาผ่าน **SSE Stream** ว่า:  
     *“ขอยังไม่ตอบเป็นข้อความ แต่ขอเรียกใช้เครื่องมือ `read_file('index.html')` ก่อน เพื่อดูเนื้อหาไฟล์”*

4. **วินาทีที่ 3 (ลงมืออ่านไฟล์จริง):**  
   - **`src/agent.ts`:** ได้รับคำขอ `tool_call` จึงสั่งหยิบเครื่องมือใน **`src/tools.ts`** มาทำงาน
   - **`src/tools.ts` (กล่องเครื่องมือ):** เปิดอ่านไฟล์ `index.html` บนฮาร์ดดิสก์ของเครื่องจริง
   - ส่งเนื้อหาไฟล์กลับมาให้ `agent.ts` เพื่อจดบันทึกเป็น **`tool_result`** ลงในสมุดโน้ต `Context`

5. **วินาทีที่ 4 (ถาม AI ซ้ำรอบที่ 2 พร้อมเนื้อหาไฟล์):**  
   - **`src/agent.ts`:** วนลูปกลับขึ้นไปถาม AI อีกครั้ง โดยครั้งนี้สมุดโน้ตมีเนื้อหาไฟล์ `index.html` แนบไปด้วย
   - สมอง AI อ่านโค้ดแล้วบอกว่า:  
     *“เจอคำว่า bug แล้ว! ขอสั่งใช้เครื่องมือ `edit(path='index.html', old_string='bug', new_string='fix')`”*

6. **วินาทีที่ 5 (ลงมือแก้ไขไฟล์จริงบนดิสก์):**  
   - **`src/tools.ts`:** ตรวจสอบว่าคำว่า `bug` มีเพียง 1 จุดหรือไม่ เมื่อตรงเงื่อนไข จึงเขียนทับเฉพาะจุดนั้นให้กลายเป็น `fix` บนฮาร์ดดิสก์
   - ส่งผลลัพธ์กลับมาว่า: *“แก้ไขสำเร็จเรียบร้อยแล้ว 1 จุด”*

7. **วินาทีที่ 6 (แจ้งผลลัพธ์ผู้ใช้):**  
   - **`src/agent.ts`:** ส่งผลสำเร็จกลับไปบอก AI
   - คราวนี้ AI ตรวจสอบว่างานเสร็จแล้ว จึงตอบเป็นข้อความ (ไม่ใช่คำสั่ง Tool แล้ว):  
     *“ผมได้ทำการแก้ไขคำว่า bug เป็นคำว่า fix ในไฟล์ index.html ให้เรียบร้อยแล้วครับ!”*
   - ข้อความนี้ถูกสตรีมทีละคำ (**`text_delta`**) ผ่าน `tui.ts` วิ่งออกหน้าจอของคุณแบบเรียลไทม์

8. **วินาทีที่ 7 (บันทึกประวัติและคืนอิสระ):**  
   - **`src/cli.ts`:** นำข้อความที่เกิดขึ้นทั้งหมดในรอบนี้ ไปเขียนต่อท้ายไฟล์ประวัติ **`~/.nanopi/session.jsonl`**
   - **`src/tui.ts`:** ปรับสถานะกลับมาเป็น **`busy = false`** ปลดล็อกหน้าจอ Terminal เพื่อรอรับคำสั่งใหม่จากคุณ!

---

### ศัพท์เทคนิคที่มือใหม่มักสงสัย (แปลและเปรียบเทียบ):

| คำศัพท์ภาษาอังกฤษ | ความหมายภาษาไทย | เปรียบเทียบกับชีวิตจริง |
| :--- | :--- | :--- |
| **SSE (Server-Sent Events)** | การสตรีมข้อมูลทางเดียวจากเซิร์ฟเวอร์ | เหมือน **"เปิดก๊อกน้ำประปา"** ข้อมูลไหลมาเรื่อยๆ ไม่ต้องรอ 30 วินาทีให้มาทีเดียวทั้งถัง |
| **text_delta** | ชิ้นส่วนตัวอักษรที่เพิ่มขึ้นทีละนิด | เหมือน **"หยดน้ำ"** แต่ละหยดที่ไหลออกมา เช่น "ส", "วัส", "ดี" |
| **tool_call** | คำสั่งที่ AI ขอให้โปรแกรมหยิบเครื่องมือ | เหมือนช่างบอกผู้ช่วยว่า **"ส่งประแจเบอร์ 10 มาหน่อย"** |
| **while loop** | ลูปวนซ้ำจนกว่าเงื่อนไขจะหยุด | เหมือนการเช็กงานซ้ำ: **"ทำเสร็จรึยัง? ถ้ายัง ทำต่อ -> วนกลับมาเช็กใหม่"** |
| **max_tokens** | ขีดจำกัดความยาวของคำตอบที่โมเดลส่งได้ | เหมือน **"หมึกพิมพ์หมดกลางประโยค"** คำสั่งอาจขาด จึงต้องมีระบบความปลอดภัยห้ามรันคำสั่งที่ขาดเด็ดขาด |
| **Abort / AbortSignal** | กลไกดักจับการยกเลิกงาน | เหมือน **"ปุ่มเบรกฉุกเฉินสีแดง (Emergency Stop)"** เมื่อเรากด `Ctrl+C` ระบบจะหยุดงานทันทีอย่างปลอดภัย |
| **Truncate 200 บรรทัด** | การตัดความยาวข้อความให้เหลือ 200 บรรทัด | เหมือนการ **"ย่อหนังสือเล่มหนาให้เหลือแค่ข้อสรุป 2 หน้าสุดท้าย"** เพื่อไม่ให้สมุดโน้ตล้นและประหยัดค่า Token |
| **Busy State** | สถานะกำลังประมวลผล | เหมือนขึ้นป้ายหน้าร้านว่า **"กำลังให้บริการ กรุณารอสักครู่"** ป้องกันไม่ให้ลูกค้าคนอื่นเดินมาสั่งซ้อน |
| **Composition Root** | จุดเริ่มต้นที่ต่อสายอุปกรณ์ทุกตัวเข้าด้วยกัน | เหมือน **"ตู้ควบคุมไฟฟ้าหลัก (Main Board)"** ที่เปิดสวิตช์แล้วไฟสว่างทั่วทั้งบ้าน |
| **Session JSONL** | ไฟล์บันทึกประวัติแบบ 1 บรรทัดต่อ 1 ข้อความ | เหมือน **"สมุดบันทึกรายวัน"** ที่เขียนต่อท้ายทุกวัน ปิดโปรแกรมแล้วเปิดใหม่ ประวัติก็ไม่สูญหาย |
